...
const School = props => {
  ...
  const { MySchool } = useSelector(state => ({
    MySchool: state.Schools?.data,
  }))
  ...
  const handleUpdateSubmit = (e, values) => {
    if (isEdit) {
      ...
      dispatch(onUpdateSchool(updatedSchool))
    } else {
      const newSchool = {
        name: values["name"],
      }
      dispatch(onAddSchool(newSchool))
    }
  }
  const columns =[
    {
  ...
  const pageOptions = {
    sizePerPage: 10,
    totalSize:  MySchool?.size, 
    custom: true,
  }

  const handleDeleteSubmit = item => {
    dispatch(onDeleteSchool(item?._id))
    console.log("silme fonksiyonuna geldi", item?._id)
  }
  ...
  return (<Container fluid>
    ...
    <Breadcrumb title={props.t("Okullar")} breadcrumbItem={props.t("Okullar")} addBtn={true} addBtnCallBack={() => {
              setIsEdit(false);
              checkToggle();
              setSelected("");
        ...
        {confirm_alert ? (
              <SweetAlert
                  ...
                  onConfirm={() => {
                      ...
                      dispatch(onDeleteSchool(selectedId));
                      setSelectedId(null);
                  }}
...
export default withRouter(withTranslation()(School))
