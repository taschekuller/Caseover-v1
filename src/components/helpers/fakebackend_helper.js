    ...
    .catch(err => {
      ...
      if (err.response && err.response.status) {
        switch (err.response.status) {
          case 404:
            message = "Sorry! the page you are looking for could not be foundccc"
            break
          case 500:
            message =
  ...
  return axios
    ...
    .catch(err => {
      ...
      if (err.response && err.response.status) {
        switch (err.response.status) {
          case 404:
            message = "Sorry! the page you are looking for could not be foundxxx"
            break
          case 500:
            message =
              "Sorry! something went wrong, please contact our support team"
...
export const deleteCompany= company =>
  del(url.DELETE_COMPANY, { headers: { company } })  
